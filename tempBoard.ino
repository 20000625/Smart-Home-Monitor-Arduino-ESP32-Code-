...
void loop() {
  if (!ipReceived) {
    const char* message = "Radiator"; // Message to broadcast
    ...
    Serial.println("Broadcasted UDP message: Radiator");
  ...
  if (packetSize) {
    ...
    if (command == "ON") {
      ...
      if (temp < minValue) {
        ...
        Serial.println("Temp below desired levels, LED ON.");
      } else {
        digitalWrite(ledPin, LOW);   // Turn the LED off
        Serial.println("Temp above desired levels, LED OFF.");
      }
    }
  }
  ...
  if (client.connected() && client.available()) {
    ...
    if (ledControlAuto) {
      if (temp < minValue) {
        digitalWrite(ledPin, HIGH); // Turn the LED on
        Serial.println("Temp levels are too low, LED ON.");
      } else {
        digitalWrite(ledPin, LOW); // Turn the LED off
        Serial.println("Temp levels are acceptable, LED OFF.");
      }
    }
  }
