...
void loop() {
  if (!ipReceived) {
    const char* message = "Humidifier"; // Message to broadcast
    ...
    Serial.println("Broadcasted UDP message: Humidifier");
  ...
  // Open TCP connection if IP is received
    ...
    if (!client.connected()) {
      ...
      if (client.connect(remoteIp, remotePort)) {
        ...
        return;  // Retry on failure
            ...
            // Send "TEMP" message to request temperature data
    client.println("HUMIDITY");
    Serial.println("Sent HUMIDITY request to server");

    lastRequestTime = millis();
  }

  // Listen for incoming messages from the TCP server
  if (client.connected() && client.available()) {
    String message = client.readStringUntil('\n'); // Read the message until newline
    Serial.print("Received message: ");
    Serial.println(message);  // Print the received message (temperature data in string)

    // Trim any leading/trailing whitespace or newline characters
    message.trim();
    Serial.print("Trimmed message: ");
    Serial.println(message);  // Print the trimmed message for debugging

    // Convert the received string to a double
    humidity = message.toDouble();
    Serial.print("Converted humidity: ");
    ...
    if (ledControlAuto) {
      if (humidity < minHumidity || humidity > maxHumidity) {
        digitalWrite(ledPin, HIGH);  // Turn the LED on
        Serial.println("Temperature below threshold, LED ON.");
      } else {
        digitalWrite(ledPin, LOW);   // Turn the LED off
        Serial.println("Temperature above threshold, LED OFF.");
      }
    }
  }

  delay(10); // Wait 1 second before checking again
}